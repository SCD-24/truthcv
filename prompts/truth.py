...
def extract_system() -> str:
    ...
    return (
        ...
        "Capture education as degree + school + start/end, a flat list of skills, and a "
        "flat list of hobbies/interests only if the text has such a section (empty list "
        "otherwise). Also extract a top-level profile header, copying VERBATIM (no inference): "
        "the person's name, email, phone, location, any profile links (each as label + url), "
        ...
        "Do not duplicate bullets, skills or hobbies."
    )
