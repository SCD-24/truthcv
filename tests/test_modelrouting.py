...
def test_from_dict_ignores_legacy_context_window(data_dir):
    route = Route.from_dict(
        {"connection": "claude", "model": "claude-opus-4-8", "context_window": 200000}
    )
    assert route == Route("claude", "claude-opus-4-8")
    assert route.to_dict() == {"connection": "claude", "model": "claude-opus-4-8", "effort": ""}
