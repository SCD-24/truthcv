<!-- generated:start edge:screening-engine→truth-data-volume -->
# Interaction: Screening Engine → Truth Data Volume

- **Participants:** `screening-engine` → `truth-data-volume`
- **Transport:** `file I/O`
<!-- generated:end edge:screening-engine→truth-data-volume -->

## Contract

_TODO (agent-owned): request/response shapes and invariants._

## Auth

_TODO (agent-owned): how this interaction is authenticated._

## Flow

_TODO (agent-owned): the sequence of the interaction._

## Code

_TODO (agent-owned): entry points on both sides._
